...
function WebsitePreviewCard({ item, index }: { item: { label: string; category: string; liveUrl: string }; index: number }) {
    ...
    <a href={item.liveUrl} target="_blank" rel="noopener noreferrer" className="group block">
      <div className="border border-charcoal-100 mb-4 overflow-hidden transition-all duration-300 group-hover:border-brand-300 group-hover:shadow-lg">
        <div className="bg-charcoal-100 px-4 py-2.5 flex items-center gap-3 border-b border-charcoal-200">
          ...
          <div className="flex-1 bg-white rounded-md px-3 py-1 text-xs text-charcoal-500 truncate">{domain}</div>
        </div>
        <div className={`aspect-[4/3] bg-gradient-to-br ${colorClass} relative`}>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-white/80 p-6">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="mb-2 opacity-60">
              <circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
            </svg>
            <span className="text-sm font-medium text-center opacity-80">{item.label}</span>
          </div>
...
const workTypes = [
  { label: "Naeem Clinic", category: "Website", liveUrl: "http://naeemclinic.great-site.net/" },
  { label: "ClickForce Agency", category: "Website", liveUrl: "https://clickforceagency.great-site.net/" },
  { label: "Asad Unani Shop", category: "E-Commerce", liveUrl: "https://asadunanishop.infinityfreeapp.com/" },
  { label: "Aurangzaib Garments", category: "Website", liveUrl: "https://aurangzaibgarments.netlify.app/" },
            ...
            {workTypes.map((item, i) => (
              <AnimatedSection key={item.label} delay={i * 0.08}>
                {"liveUrl" in item && item.liveUrl ? (
                  <WebsitePreviewCard item={item as { label: string; category: string; liveUrl: string }} index={i} />
                ) : (
